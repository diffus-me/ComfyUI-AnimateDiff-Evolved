...
class CheckpointLoaderSimpleWithNoiseSelect(io.ComfyNode):
    ...
    def define_schema(cls, exec_context: execution_context.ExecutionContext) -> io.Schema:
        return io.Schema(
            ...
            category='Animate Diff 🎭🅐🅓/extras',
            inputs=[io.Combo.Input('ckpt_name', options=folder_paths.get_filename_list(exec_context, "checkpoints")), io.Combo.Input('beta_schedule', options=['autoselect', 'use existing', 'sqrt_linear (AnimateDiff)', 'linear (AnimateDiff-SDXL)', 'linear (HotshotXL/default)', 'avg(sqrt_linear,linear)', 'lcm avg(sqrt_linear,linear)', 'lcm', 'lcm[100_ots]', 'lcm >> sqrt_linear', 'sqrt', 'cosine', 'squaredcos_cap_v2'] , default='use existing'), io.Boolean.Input('use_custom_scale_factor', default=False, optional=True), io.Float.Input('scale_factor', default=0.18215, max=1.0, min=0.0, step=1e-05, optional=True)],
            outputs=[io.Model.Output('MODEL'), io.Clip.Output('CLIP'), io.Vae.Output('VAE')],
            hidden=[io.Hidden.exec_context],
        )
    @classmethod
    def execute(cls, ckpt_name, beta_schedule, output_vae=True, output_clip=True, use_custom_scale_factor=False, scale_factor=0.18215, exec_context: execution_context.ExecutionContext=None):
        ckpt_path = folder_paths.get_full_path(exec_context, "checkpoints", ckpt_name)
